...
- **Code quality**: dead code, unclear naming, swallowed or generic error handling, and duplicated logic that should be shared. Also flag anything that contradicts the codebase's existing conventions. Use the `clean-code` skill in this plugin as your reference for code-level quality, covering magic values, cognitive complexity, naming, error handling, and the like. Where `clean-architecture` governs the layers, `clean-code` governs what happens inside them.
