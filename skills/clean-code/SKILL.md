...
Never pass a raw numeric literal into a function call, a comparison, or an assignment unless the meaning is immediately obvious from context. Numbers like `0`, `1`, `true`, and `false` are generally fine. Anything else gets a named constant.
...
Optimize for the total cost of understanding the code, not for any single metric. That total is the local complexity of the function you are reading, plus the cost of jumping to everything it calls to follow the logic. Line count alone measures neither.

The real signals of high complexity are deep nesting and deep boolean expressions, because both force a reader to hold several conditions in their head at once.
...
- If a conditional has more than two or three terms joined by `&&` or `||`, extract it into a named boolean or a small function that describes the intent.
...
Handle edge cases, invalid input, and error conditions at the top of a function and return early, instead of wrapping the main logic in a nested `if`. This keeps the happy path at the lowest indentation level, where it is easiest to read, and it is the highest-leverage way to remove nesting.
...
Real duplication, the same logic or the same rule expressed in two places that must stay in sync, should be pulled into one place. When that rule changes, you want to change it once.

But a wrong abstraction costs more than a little repetition. Two pieces of code that look alike today are not always the same thing, and forcing them under one abstraction couples them so that a change to one drags the other along. Wait until the duplication is proven, and the shared concept is real, before unifying. If you find yourself adding flags or branches to a shared function so it can serve two callers that are drifting apart, that is a sign the abstraction was wrong. Prefer the duplication.

## Too many parameters

A long parameter list is a smell. It usually means a missing type that should group the related arguments, or a function that is doing too much and should be split. It also makes call sites hard to read, especially when several parameters share a type and can be swapped by mistake.
